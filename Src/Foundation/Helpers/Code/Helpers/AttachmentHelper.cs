...
namespace DEWAXP.Foundation.Helpers
{
	...
	{
        public static bool AttachmentIsValid(HttpPostedFileBase attachment, long maxBytes, out string error, params string[] allowedExtensions)
        {
            error = null;
        ...
        public static string GetListMimeType(string extension)
        {
            ...
            foreach (var ext in lstextension)
            {
                string regext = Regex.Replace(ext, "[^a-zA-Z]+", string.Empty);
                if (!string.IsNullOrWhiteSpace(regext))
                {
                    returnstr.Add(GetMimeType(regext.Trim()));
                }
            }
            return string.Join(",", returnstr.Distinct());
        }
    ...
    public static class BillingImageHelper
    {
