...
helix.header("DEWA XP",
    "This is a Buid action for DEWA XP");
...
var publishProjects = function (location, dest) {
    ...
    root = location;
    roots = "/**/*.csproj";
    console.log("publish to " + dest + " folder"+ root+roots);
...
gulp.task("Build-Solution", function (callback) {
    ...
    var overrideParams = [];
    overrideParams.push('/p:Configuration=' + config.buildConfiguration);
    overrideParams.push('/p:DeployOnBuild=false');
    msbuild.config('overrideParams', overrideParams);
    msbuild.config('version', config.buildToolsVersion);
    msbuild.build();
});
...
gulp.task("Auto-Publish-Assemblies",
    function() {
        var root = "./src";
        var roots = [root + "/**/code/**/bin", root + "/**/**/code/**/bin"];
        var files = "/**/DEWAXP.{Feature,Foundation,Project}.*.{dll,pdb}";;
...
gulp.task("Package-Prepare-Package-Files",
    function(callback) {
        var excludeList = [
            ...
            "!" + config.websiteRoot + "\\bin\\DEWAXP.{Feature,Foundation,Project,Demo,Common}*dll"
        ];
